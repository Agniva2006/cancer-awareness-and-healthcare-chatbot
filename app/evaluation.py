...
from app.main import register, login, RegisterRequest, LoginRequest, PredictionRequest
...
reg_req = RegisterRequest(username="test_oncologist", password="securepassword123", email="test@oncology.org")
reg_res = register(reg_req)
test("User registration status", lambda: reg_res["success"] == True or "already" in reg_res.get("message", ""))

login_req = LoginRequest(username="test_oncologist", password="securepassword123")
login_res = login(login_req)
test("User login authorization token", lambda: login_res.get("success") == True and ("access_token" in login_res or "token" in login_res))
